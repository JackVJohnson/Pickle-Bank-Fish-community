...
df <- read.csv(file.path(data_directory, 'pickle_bank_fish_data.csv'))
df <- df[,c(1:19)]
df <- na.omit(df)
...
p_ord <- ggplot(data=scores,aes(x=MDS1, y=MDS2, colour=depth, fill = depth)) +
  ...
  annotate("text", x = 0.75, y = -.9, label = "Stress = 0.17", size = 6)
p_ord
...
p3 <- ggplot(trophic_df, aes(foodweb, sum_abundance)) +
  geom_boxplot() +
  my_theme +
  labs(x='Foodweb', y='Total abundance') +
...
p_trophic_ord <- ggplot(data=scores2,aes(x=MDS1, y=MDS2, colour=depth, fill = depth)) +
  ...
  scale_fill_manual(values=c('#87CEEB','#4169E1')) +
  labs(x='MDS1', y='MDS2', fill = 'Depth (m)', colour = 'Depth (m)') +
   annotate("text", x = 0.5, y = -.4, label = "Stress = 0.117", size = 6)
p_trophic_ord
...
png(file=file.path(output_directory, "trophic_plots.png"), height = 4000, width = 3500, res = 350)
(p1 + p2) / (p3 + p4) / (p_trophic_ord + guide_area() +
  plot_layout(guides = 'collect')) + 
  plot_annotation(tag_levels = list(c('(A)', '(B)', '(C)', '(D)', '(E)', ''))) & theme(plot.tag = element_text(size = 30, face = "bold"))
dev.off()

tiff(file=file.path(output_directory, "trophic_plots.tif"), height = 4000, width = 3500, res = 350)
(p1 + p2) / (p3 + p4) / (p_trophic_ord + guide_area() +
  plot_layout(guides = 'collect')) + 
  plot_annotation(tag_levels = list(c('(A)', '(B)', '(C)', '(D)', '(E)', ''))) & theme(plot.tag = element_text(size = 30, face = "bold"))
dev.off()
